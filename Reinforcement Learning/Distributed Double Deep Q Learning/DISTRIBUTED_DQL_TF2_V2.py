...
class Server:
    def __init__(self):
        ...
        self.index = 0
        if not os.path.isdir(f'server/{MODEL_NAME}_0/{self.index}'):
            os.makedirs(f'server/{MODEL_NAME}_0/{self.index}')    
            
        self.model.save(f'server/{MODEL_NAME}_0/{self.index}/main')
        self.target_model.save(f'server/{MODEL_NAME}_0/{self.index}/target')
