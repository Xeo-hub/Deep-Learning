...
class CarEnv:
    ...
    def step(self, action):
        ...
        return self.front_camera, reward, done, self.index
...
class Server:
    def __init__(self):
        ...
        self.model.save(f'server/{MODEL_NAME}/{self.index}/main')
        self.target_model.save(f'server/{MODEL_NAME}/{self.index}/target')
...
class DQNAgent:
    def __init__(self, identifier, gpu):
        self.gpu = gpu  
        self.identifier = identifier
        self.index = 0
        self.model = tf.keras.models.load_model(f'server/{MODEL_NAME}_0/{self.index}/main)
        self.target_model = tf.keras.models.load_model(f'server/{MODEL_NAME}_0/{self.index}/target')
        ...
        self.tensorboard = ModifiedTensorBoard(self.model, log_dir=f"agents/logs/{self.index}/{self.identifier}/{MODEL_NAME}")
        self.terminate = False
        self.last_logged_episode = 0
        self.training_initialized = False

    def update_replay_memory(self, transition):
        # ESTO HABRÁ QUE REVISARLO SEGUN VAYA A HACERLO DISTRIBUIDO (SI HAGO UN REPLAY PARA CADA UNO, HABRÁ QUE PONERLO EN CARENV Y QUE CADA UNO MODIFIQUE SU MODELO DESDE AHI)
        # transition = (current_state, action, reward, new_state, done, index)
...
def mean_weights (agents):
    # Promediar los pesos de cada agente
    weights = [agent.model.weights for agent in agents]
...
if __name__ == '__main__':
    ...
    for i in range (num_agents):
        # To identify every agent and env
        # NOTE: REMEMBER TO CREATE n-1 SERVERS OF CARLA WITH THE COMMAND ./... : https://github.com/carla-simulator/carla/issues/3238
        # AND MAKE THEM LISTEN IN PORTS 2000...2000+n-1
        agent = DQNAgent(i, gpus[i])
        env = CarEnv(i, 2000+i)
        agents[i] = (agent, env)
        
    # Start training thread and wait for training to be initialized
    for i in range (len(agents)):
        threads.append(Thread(target=agents[i][0].train_in_loop, daemon=True))
    ...
    for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
        # NOTE: CADA 500 EPISODIOS ACTUALIZAMOS EL MODELO PRINCIPAL CON LA MEDIA DE LOS PESOS DE LOS AGENTES Y CARGAMOS EL NUEVO MODELO EN LOS AGENTES
        if episode % 500 == 0 and episode != 0:
            server.index += 1
            new_weights = mean_weights(agents)
            server.model.set_weights(new_weights)
            server.target_model.set_weights(new_weights)
            self.model.save(f'server/{MODEL_NAME}/{self.index}/main')
            self.target_model.save(f'server/{MODEL_NAME}/{self.index}/target')
            
            for i in range (len(agents)):
                agent = agents[i][0]
                agent.model = tf.keras.models.load_model(f'server/{MODEL_NAME}/{self.index}/main')
                agent.target_model = tf.keras.models.load_model(f'server/{MODEL_NAME}/{self.index}/target')
            
            
        # NOTE: ESTO HABRIA QUE HACERLO PARA QUE CADA GPU LO EJECUTE CON UN (AGENT, ENVT)
        for i in range (len(agents)):
            agent = agents[i][0]
            env = agents[i][1]
